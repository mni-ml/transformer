 ...
 *
 * MLA compresses the K/V path through a low-rank latent `c_kv` (dim d_c << n_embd).
 * Per-head K and V are reconstructed from c_kv via learned up-projections. Q stays
 * as a dense projection; absolute position embeddings stay the same. At inference
 * this means a KV cache of d_c floats per token instead of 2*n_embd — ~6x smaller
 * at d_c=128, n_embd=384.
 *
 * Expects pre-tokenized binary data produced by prepare_tinystories.py:
...
const CONFIG = {
  ...
  nHead: 6,
  nLayer: 6,
  dKv: 128,            // MLA latent dim (d_c). KV cache per token = dKv floats.
  blockSize: 256,
  batchSize: 64,
  gradAccumSteps: 1,
  maxIters: 5000,
...
// ════════════════════════════════════════════════════════════════
// LayerNorm (shared with train_youtube.js)
// ════════════════════════════════════════════════════════════════
...
// ────────────────────────────────────────────────────────────────
// K and V are produced from a shared low-rank latent c_kv = LN(x · W_DKV),
// with per-head K/V materialised via W_UK / W_UV at compute time. At
// inference we'd cache only c_kv (d_c floats/token) instead of full K,V
// (2·nEmbd floats/token). Q is a regular dense projection (no low-rank
// compression). No RoPE — absolute position embeddings handle position.
// ════════════════════════════════════════════════════════════════

class MLACausalSelfAttention extends Module {
  constructor(nEmbd, nHead, dKv, dropoutRate) {
    super();
    if (nEmbd % nHead !== 0) {
  ...
  forward(x) {
    ...
    const cKv = this.kvLatentNorm.forward(this.kvDownProj.forward(x));
    let k = this.keyUpProj.forward(cKv);
    let v = this.valueUpProj.forward(cKv);
...
class TransformerBlock extends Module {
  constructor(nEmbd, nHead, dKv, dropoutRate) {
    ...
    this.attn = new MLACausalSelfAttention(nEmbd, nHead, dKv, dropoutRate);
...
class MiniGPT extends Module {
  constructor(vocabSize, config) {
    super();
    const { nEmbd, nHead, nLayer, dKv, blockSize, dropoutRate = 0 } = config;
    if (dKv == null) {
      throw new Error('config.dKv is required for MLA MiniGPT');
    }
    this.config = config;
    this.vocabSize = vocabSize;
    this.tokenEmb = new Embedding(vocabSize, nEmbd);
    this.posEmb = new Embedding(blockSize, nEmbd);
    for (let i = 0; i < nLayer; i++) this[`block${i}`] = new TransformerBlock(nEmbd, nHead, dKv, dropoutRate);
...
function main() {
  console.log('');
  console.log('═══════════════════════════════════════════════════════');
  console.log('  MiniGPT — Multi-head Latent Attention (MLA)');
  console.log('═══════════════════════════════════════════════════════');
  console.log('');
  ...
  if (!existsSync(metaPath)) {
    console.error('  ERROR: data/meta.json not found — run prepare_tinystories.py first');
    process.exit(1);
  }
  ...
  const kvCacheBytesPerTok = CONFIG.dKv * 4;
  const baselineKvBytesPerTok = 2 * CONFIG.nEmbd * 4;
  const compressionRatio = (baselineKvBytesPerTok / kvCacheBytesPerTok).toFixed(1);
  ...
  console.log(`  Architecture:   ${CONFIG.nLayer} blocks, ${CONFIG.nHead} heads, ${CONFIG.nEmbd}-dim, d_c=${CONFIG.dKv} (MLA)`);
  console.log(`  Context window: ${CONFIG.blockSize} tokens`);
  console.log(`  Dropout:        ${CONFIG.dropoutRate}`);
  console.log(`  KV cache @ inf: ${kvCacheBytesPerTok} B/token/layer vs baseline ${baselineKvBytesPerTok} B (${compressionRatio}x smaller)`);
